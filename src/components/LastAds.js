...
const LastAds = (props) => {
  ...
  return (
    <div className='container mt-3'>
      {props.loading ? <LinearProgress style={{marginTop:'150px'}} color='secondary' /> : (<>
      <Typography variant = 'h4'> Bütün Elanlar </Typography>
      <Divider style={{maxWidth:'92%'}} />
      <br/>
      <Grid container spacing={3}>
        { props.loading ? <Progress/> : adsSlice && adsSlice.map((ad,index) => {return <Ad key={ad.id} index={index} {...ad} />}) }
      </Grid>
...
const mapStatetoProps = (state) => ({
  ads : state.adsReducer.slice(0).reverse(),
